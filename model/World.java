    ...
    private Color trace(Ray r, int cutoff, double nju1, Shape ins, Graphics g){
        ...
        if (s != null){
            ...
            if(s.refraction() > 0) {
            	...
            	if (s == ins) {
            		nT = Point3D.zero.minus(N);
            		//nju2 = nju1;
            		//nju2 = 1;
            		
            	}
                double nju = nju1 / nju2;
                double c1 = Point3D.zero.minus(nT).dot(r.getVector());
                double c2 = Math.sqrt(1 - nju*nju*(1 - c1*c1));
                Point3D ipar = nT.multiply(-c1);
                Point3D iperp = r.getVector().plus(nT.multiply(c1));
                Point3D t = r.getVector().multiply(nju).plus(nT.multiply(nju*c1 - c2));
                //Refractive
                //System.out.println(t.getX() + " - " + t.getY() + " - " + t.getZ());
                Color c = trace(new Ray(point.plus(t.multiply(1)), t), cutoff - 1, s.refraction(), s, g);
...
/*                
                ...
                g.drawLine((int)worldX(point.getX()), (int)worldY(point.getY()), (int)worldX(point.plus(t.multiply(1)).getX()), (int)worldY(point.plus(t.multiply(1)).getY()));
                */
                /*try {
					Thread.sleep(1);
